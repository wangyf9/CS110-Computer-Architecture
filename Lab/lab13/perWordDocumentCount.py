...
def flatMapFunc(document):
    ...
    ret = []
    for i in words:
        ret.append((i,documentID))## in this step we can write down every pair in this document, that is word and its corresponding documentID 
    return ret
...
def mapFunc(arg):
    ...
    """
    return (arg[0], 1)##because we append (i,documentID) in the setting, and because we just need to calculate the total num of the appearence of word, so we donnot need to store the ID of the document.
...
def perWordDocumentCount(file_name, output="spark-wc-out-perWordDocumentCount"):
    ...
    words = file.flatMap(flatMapFunc).distinct() ##read from the document, that is creating the first mapping relation, and use distinct function to merge the same elements, since if one word appears in the first document many times we just need to note it as one time.
    difference = words.map(mapFunc) ##create second pairs, that is the final mapping relation
    result= difference.reduceByKey(reduceFunc).sortByKey() ##merge the element with the same key that is the same words and sort them.
    result.coalesce(1).saveAsTextFile(output)   ##save
