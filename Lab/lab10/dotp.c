...
double dotp_manual_optimized (double *x, double *y, int arr_size)
{
  ...
  #pragma omp parallel
  {
    ...
    int block_size = arr_size/num_thread;
    for (int i = thread_id*block_size; i < (1+thread_id)*block_size; i++){
      local_sum += x[i] * y[i];
    }
    /*special case, marginal case*/
    if(thread_id==num_thread-1 && (1+thread_id)*block_size<arr_size){
      for (int i=(1+thread_id)*block_size; i<arr_size; i++){
        local_sum+= x[i] * y[i];
      }
    }
