...
static void *threadpool_thread(void *threadpool) {
    ...
    while (true) {
        ...
        if (pool->shutdown) {
            // TODO: sync?
            if(pool->sync==false||ringbuffer_is_empty(pool->task_list)==1){
                lock_release(pool);
                pthread_exit(NULL);
            }
            if(pool->sync==true){
        ...
        if(ringbuffer_pop(pool->task_list,&new_task)==0){
            continue;
        }
        /* 6. Unlock */
...
bool threadpool_add_task(threadpool_t *pool, void (*func)(void *), void *args) {
    ...
    /* 1. Acquire lock */
    if (!lock_acquire(pool)) { return false; }/*locking down*/
    ...
    new_task.func=func;
    new_task.args=args;
    if(ringbuffer_push(pool->task_list, new_task)==0||pool->shutdown==1){
        lock_release(pool);
        return false;
    }
