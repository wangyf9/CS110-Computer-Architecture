...
bool explorer_cdpar(struct explorer *exp) {
  /*the parent directory does not exists, return false*/
  if (!exp || exp->cwd == exp->root) {
    return false;
  }
  /*Change directory to the parent directory of current working
 * directory*/
...
bool explorer_chdir(struct explorer *exp, const char *name) {
  ...
  /*assignment*/
  sub = dir_find_node(exp->cwd, name);
  /*determine whether this directory exists and whether is a dir*/
  if (!sub || !sub->is_dir) {
    return false;
  }
...
bool explorer_support_filetype(struct explorer *exp, open_func callback,
                               int filetype) {
                                /*Check for null pointer and 
                                the correct range of file type*/
  if (!exp || callback == NULL || filetype < 0 || filetype >= MAX_FT_NO) {
    return false;
  }
  /*we already have a callback function for `filetype`, return false*/
  if (exp->ft_open[filetype] != NULL) {
    return false;
  }
  /*success Register a new callback function for filetype number*/
...
bool explorer_open(const struct explorer *exp, const char *name) {
  ...
  sub = dir_find_node(exp->cwd, name);
  /*we are trying to open a directory, return false*/
  if (!sub || sub->is_dir) {
    return false;
  }
  file = sub->inner.file;
  /* we don't have a callback function for the, return false*/
...
bool explorer_contain(struct explorer *exp, const char *name) {
  /*determine whether it contain someting which are not null pointer*/
